...
foreach($name in $targets){
  ...
  $title = if($tm.Success){$tm.Groups[1].Value.Trim()} else {[IO.Path]::GetFileNameWithoutExtension($name)}

  $cfgFiles = New-Object System.Collections.Generic.List[string]
  $mCfg = [regex]::Match($raw,'(?s)## Configuration Files\s*(.*?)\n\n## ')
  if($mCfg.Success){ foreach($ln in ($mCfg.Groups[1].Value -split "`r?`n")){ $t=$ln.Trim(); if($t.StartsWith('- ')){ [void]$cfgFiles.Add($t.Substring(2).Trim().Trim('`')) } } }
  ...
  foreach($sec in $fileSections){
    ...
    foreach($ln in $lines){
      ...
      if($trim -match '^([^:]+):\s*(.*)$'){
        ...
        $kp=((@($prefix)+$k) -join '.').Trim('.')
        if($kp -eq ''){ continue }
        [void]$rows.Add('- ' + $kp + ' - ' + (Get-KeyDesc $kp) + ' ' + (Infer-Type $v) + ' Recommended baseline: ' + $v + '.')
      }
    }
  ...
  try { Set-Content $path $newText -Encoding UTF8 }
  catch { Write-Output ('failed:' + $name) }
}
Write-Output 'ok'
